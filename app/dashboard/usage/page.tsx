...
export default function UsagePage() {
  ...
  if (!usage) {
    return <div className="text-center text-gray-500">Failed to load usage data</div>;
  }
  ...
  return (
    <div>
      <div className="mb-8">
        <h1 className="text-4xl font-bold text-gray-900 mb-2">Usage Statistics</h1>
        <p className="text-gray-500">Monitor your token and message usage</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        <div className="border border-gray-200 rounded-lg p-6 bg-white shadow-sm">
          ...
          <div className="space-y-3">
            <div className="flex justify-between">
              ...
              <span className="font-semibold text-gray-900">{usage.today.tokens.toLocaleString()}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-500">Messages</span>
              <span className="font-semibold text-gray-900">{usage.today.messages.toLocaleString()}</span>
            </div>
          </div>
        </div>
        ...
        <div className="border border-gray-200 rounded-lg p-6 bg-white shadow-sm">
          ...
          <div className="space-y-3">
            <div className="flex justify-between">
              <span className="text-gray-500">Tokens</span>
              <span className="font-semibold text-gray-900">{usage.thisMonth.tokens.toLocaleString()}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-500">Messages</span>
              <span className="font-semibold text-gray-900">{usage.thisMonth.messages.toLocaleString()}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      {usage.daily.length > 0 && (
        <div className="border border-gray-200 rounded-lg p-6 bg-white shadow-sm">
          <h2 className="text-xl font-bold text-gray-900 mb-4">Daily Usage (Last 7 Days)</h2>
          <div className="space-y-3">
            {usage.daily.map((day) => (
              <div key={day.date} className="flex justify-between items-center py-2 border-b border-gray-100 last:border-0">
                ...
                <div className="text-right">
                  <div className="font-semibold text-gray-900">{day.tokensUsed.toLocaleString()} tokens</div>
                  <div className="text-sm text-gray-500">{day.messageCount} messages</div>
                </div>
              </div>
            ))}
          </div>
      ...
      {usage.byAgent.length === 0 && usage.daily.length === 0 && (
        <div className="border border-gray-200 rounded-lg p-12 bg-white shadow-sm text-center">
          ...
          <p className="text-gray-500">Start using your agents to see usage statistics here</p>
        </div>
      )}
    </div>
  );
